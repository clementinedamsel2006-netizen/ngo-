...
# Keep CSV files in a "data" folder next to this Python file.
# Streamlit Cloud runs from a different working directory than a local
# machine, so NEVER hard-code paths such as /mount/src/ngo-/data.
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# Optional override for Streamlit Cloud / Docker / other deployments.
# Example: DATA_DIR=/mount/src/ngo-/data
ENV_DATA = os.environ.get("DATA_DIR", "").strip()

# Try the script directory first, then the current working directory.
DATA_CANDIDATES = []
if ENV_DATA:
    DATA_CANDIDATES.append(os.path.abspath(ENV_DATA))
DATA_CANDIDATES.extend([
    os.path.join(BASE_DIR, "data"),
    os.path.join(os.getcwd(), "data"),
])

# Select the first directory containing at least one of the expected files.
# If none exists yet, use BASE_DIR/data so local development can create it.
REQUIRED_CSVS = (
    "volunteer.csv",
    "ngo.csv",
    "opportunities.csv",
...
DATA = next(
    (d for d in DATA_CANDIDATES
     if all(os.path.isfile(os.path.join(d, f)) for f in REQUIRED_CSVS)),
    DATA_CANDIDATES[0]
)
...
try:
    ...
    st.warning(
        ...
        + "\n\n"
        "Put these CSV files inside a `data` folder in the same repository "
        "as this Streamlit app, then redeploy. "
