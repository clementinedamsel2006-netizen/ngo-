...
# All CSV files live in the SAME directory as this Streamlit app.
# No "data" folder is required. This also avoids Streamlit Cloud paths
# such as /mount/src/ngo-/data.
...
# CSV files are expected beside code.py.
DATA = BASE_DIR
...
st.markdown(
    ...
    <style>
        /* ------------------------------------------------------------
           FIX: Streamlit was inheriting a dark text theme while the app
           explicitly paints a light background. That made labels, tabs,
           headings and input text almost invisible on mobile.
           ------------------------------------------------------------ */
        ...
        .stApp {
            ...
            /* Keep Streamlit/theme-aware components on a light palette. */
            --text-color: #262730;
            --st-text-color: #262730;
            --background-color: #f7faf8;
            --st-background-color: #f7faf8;
            --secondary-background-color: #ffffff;
            --st-secondary-background-color: #ffffff;
            --border-color: #dce7e2;
            --st-border-color: #dce7e2;
        }
        ...
        /* Widget labels: Email, Password, Sign in as, etc. */
        ...
        /* Text inputs/password inputs: white field + dark readable text. */
        ...
        /* Tabs were also inheriting white text from the dark theme. */
        [data-baseweb="tab"] {
            color: #262730 !important;
        }
        ...
        /* Muted/caption text should remain visible on the light background. */
...
try:
    ...
    st.warning(
        ...
        + "\n\n"
        "Put these CSV files in the SAME directory as `code.py` in your repository, "
        "then redeploy. No `data` folder is required. "
    ...
    # ==================================================
        ...
        if selected_id is not None:
            ...
            with right:
                ...
                # Show the overall match only.  Do not expose the internal
                # semantic/skill/interest/location/availability component scores.
                st.markdown(
                    f"## {match["overall"]}% match"
                )
                st.progress(
                    max(0.0, min(1.0, float(match["overall"]) / 100.0))
                )
                st.subheader("Why this match?")

                # Keep the useful human-readable reasons, but hide the
                # internal numeric matching breakdown.
                for reason in (
                    match.get("reasons")
                    or match.get("why")
                    or match.get("explanations")
                    or []
                ):
            ...
            for row in filtered:
                ...
                with st.container(border=True):
                    ...
                    with st.expander(
                        ...
                        st.subheader(
                            str(row["match"].get("overall", row["match_total"]))
                            + "% match"
                        )
                        st.progress(
                            max(
                        ...
                        st.subheader("Why this match?")
                        for reason in (
                            row["match"].get("reasons")
                            or row["match"].get("why")
                            or row["match"].get("explanations")
                            or []
                        ):
                            st.markdown(f"✓ {reason}")
